...
void VulkanEngine::init_swapchain() {
    ...
    vmaCreateImage(_allocator, &rimg_info, &rimg_allocinfo, &_drawImage.image, &_drawImage
    .allocation, nullptr);
...
void VulkanEngine::init_commands() {
    ...
    // immediate command submit logic
    VK_CHECK(vkCreateCommandPool(_device, &commandPoolInfo, nullptr, &_immCommandPool));

    // allocate the commend buffer for immediate submits
    VkCommandBufferAllocateInfo cmdAllocInfo = vkinit::command_buffer_allocate_info
            (_immCommandPool, 1);
...
void VulkanEngine::draw()
{
    ...
    // we will overwrite it all so we dont care about what was in the older layout
...
void VulkanEngine::draw_background(VkCommandBuffer cmd) {
    ...
    // execute the compute pipeline dispatch. We are using 16x16 workgroup size so we need to
    // divide by it
...
void VulkanEngine::init_background_pipelines() {
    ...
    {
        fmt::print("Compute shader built \n");
    }
...
void VulkanEngine::init_imgui() {
    ...
    // 1: create descriptor pool for the IMGUI
